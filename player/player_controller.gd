...
func _handle_movement(delta):
	var input = (
		Vector2(
			Input.get_axis("move_left", "move_right"), Input.get_axis("move_forward", "move_back")
		)
		. normalized()
	)
	...
	var speed = SPRINT_SPEED if is_sprinting else CROUCH_SPEED if is_crouching else WALK_SPEED
