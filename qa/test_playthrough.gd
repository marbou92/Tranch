...
func _test_player_controller():
	...
	if player:
		...
		if player.WALK_SPEED == 2.8:
			_pass("Walk speed correct")
		else:
			_fail("Walk speed incorrect: %s" % player.WALK_SPEED)
		if player.CROUCH_SPEED == 1.2:
			_pass("Crouch speed correct")
		else:
			_fail("Crouch speed incorrect")
		if player.SPRINT_SPEED == 5.6:
			_pass("Sprint speed correct")
		else:
			_fail("Sprint speed incorrect")
		if player.GRAVITY == 9.8:
			_pass("Gravity correct")
		else:
			_fail("Gravity incorrect")
		if player.CROUCH_HEIGHT == 1.1:
			_pass("Crouch height correct")
		else:
			_fail("Crouch height incorrect")
		if player.STAND_HEIGHT == 1.8:
			_pass("Stand height correct")
		else:
			_fail("Stand height incorrect")
	else:
...
func _test_flashlight_system():
	...
	if player:
		...
		if player.flashlight_bat == 100.0:
			_pass("Battery starts at 100")
		else:
			_fail("Battery not at 100")
		if not player.flashlight_on:
			_pass("Flashlight starts off")
		else:
			_fail("Flashlight should start off")
	else:
...
func _test_sanity_system():
	...
	if SanitySystem:
		...
		if SanitySystem.sanity == 100.0:
			_pass("Sanity starts at 100")
		else:
			_fail("Sanity not at 100: %s" % SanitySystem.sanity)
		# Test drain rates
		if SanitySystem.DRAIN_DARK == 0.4:
			_pass("Dark drain rate correct")
		else:
			_fail("Dark drain rate incorrect")
		if SanitySystem.DRAIN_ENTITY == 1.8:
			_pass("Entity drain rate correct")
		else:
			_fail("Entity drain rate incorrect")
		if SanitySystem.RESTORE_MEDICINE == 25.0:
			_pass("Medicine restore correct")
		else:
			_fail("Medicine restore incorrect")
	else:
